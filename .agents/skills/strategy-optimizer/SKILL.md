...
分段用于控制单任务风险、便于失败局部重跑、控制数据库/worker 峰值，并避免单个全区间结果掩盖不同市场阶段。优化涉及的总区间超过 5 年时，禁止用单个回测/评估一次性跑完全部年份；必须滚动分段执行，每段约 1-2 年。即使总跨度不超过 5 年，若指数数量多、候选方向多、需要并行，或一次任务可能占满 worker，也优先分成 1-2 年段或按批次执行：
...
4. **优化会话**：长跨度建议按段分别 `optimization start --start/--end` 建会话并 evaluate，保证单次执行跨度仍在 1-2 年；若需要保留单会话的统一审计链，`--folds` 按每折 ≈ 1-2 年设定（K ≈ 总年数 ÷ 1.5，至少 3），并把折回测视为分段任务。验收只以逐折验证窗聚合为准，忽略全区间单次指标。
...
- 同步任务使用 `status --wait`，避免高频轮询；异步任务按 worker 数分批。长跨度务必按 1-2 年分段并行或分批执行。

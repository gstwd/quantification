...
- **AI 分析双调度器**: 数据摄取在 `schedule_time`（默认 17:30）执行、AI 舆情分析在 `ai_schedule_time`（默认 23:30）执行；两个调度器均为纯定时器，只入队任务。**全部数据源（指数/宏观/行业/个股）的定时摄取统一走全局数据同步调度器**，原独立的行业摄取调度器（`get_industry_scheduler` / `IndustryIngestScheduler`）与行业日频摄取链（`handle_industry_daily_ingest` 处理器、`IndustryDataService.run_daily_ingest()`）均已删除。摄取完成后由 `handle_data_sync_all` 自动入队当日 `factor_computation`。`ai_analysis_enabled=False` 时 AI 调度器不启动。

...
def upgrade() -> None:
    op.create_table(
        ...
        sa.Column(
            "status", sa.String(32), nullable=False, server_default="pending", comment="回测状态"
        ),
    ...
    op.create_table(
        ...
        sa.Column(
            "etf_code",
            sa.String(16),
            sa.ForeignKey("etf_universe.etf_code"),
            nullable=False,
            comment="ETF 代码",
        ),
        ...
        sa.Column(
            "etf_return", sa.Float, nullable=True, comment="T+1 日实际收益率（%），末日为 NULL"
        ),

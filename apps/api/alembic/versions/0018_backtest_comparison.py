...
def upgrade() -> None:
    op.create_table(
        "backtest_comparison",
        sa.Column("comparison_id", sa.String(64), primary_key=True, comment="对比唯一 ID，UUID 格式"),
        ...
        sa.Column("backtest_a_id", sa.String(64), sa.ForeignKey("backtest_run.backtest_id"), nullable=False, comment="策略 A 的子回测 ID"),
        sa.Column("backtest_b_id", sa.String(64), sa.ForeignKey("backtest_run.backtest_id"), nullable=False, comment="策略 B 的子回测 ID"),
        ...
        sa.Column("status", sa.String(32), nullable=False, server_default=sa.text("'pending'"), comment="对比状态：pending/running/success/failed/partial"),
        sa.Column("params", sa.JSON, nullable=True, comment="共享参数（基准配置、标的范围等）"),
        sa.Column("comparison_metrics", sa.JSON, nullable=True, comment="对比级别汇总指标，完成后写入"),
        ...
        sa.Column("started_at", sa.DateTime(timezone=True), nullable=True, comment="开始执行时间（UTC）"),
        sa.Column("finished_at", sa.DateTime(timezone=True), nullable=True, comment="完成时间（UTC）"),
        sa.Column("progress", sa.Integer, nullable=False, server_default=sa.text("0"), comment="对比整体进度 0-100"),
    )

...
class ShareFlowMonitorPlugin:
    ...
    def factor_definitions(self) -> list[dict]:
        return [{"factor_id": "share_delta_pct", "name": "份额日变"}, {"factor_id": "share_prob", "name": "份额概率"}]
    ...
    def run_for_universe(self, trade_date: date, universe: list[dict], context: StrategyContextData, params: dict | None = None) -> list[StrategyResult]:
        ...
        for item in universe:
            ...
            results.append(StrategyResult(trade_date=trade_date, etf_code=code, strategy_id=self.strategy_id, signal_score=score, signal_level=level, signal_label=label, factor_values=[{"factor_id": "share_delta_pct", "value": delta_pct}, {"factor_id": "share_prob", "value": score}], payload={"share_delta_pct": delta_pct}, tags=["share_flow"]))
        return results

...
包含三因子模型的评分函数：
- volume_probability：量比 → 量能概率
- direction_probability：多维方向判断 → 方向概率
- share_probability：份额变化率 → 份额概率
- composite_probability：多因子加权合成
- signal_level：综合得分 → 信号等级
"""
...
def direction_probability(
    ...
    Args:
        change_pct: ETF 当日涨跌幅（%）。
        etf_5d: ETF 近 5 日收益率（%）。
        index_5d: 基准指数近 5 日收益率（%）。
        volume_ratio: 20 日量比。
        index_change_pct: 基准指数当日涨跌幅（%）。
...
def share_probability(share_delta_pct: float | None) -> float | None:
    """将份额变化率映射为 0~100 的份额概率。

    Args:
        share_delta_pct: ETF 份额日变化率（%），None 表示无数据。

    Returns:
        份额概率，0~100；无数据时返回 None。
    """

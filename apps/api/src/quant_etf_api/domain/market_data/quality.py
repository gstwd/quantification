...
规则只依赖传入的数据行和交易日历，不访问数据库、外部 API 或服务容器。
调用方负责读取数据、记录日志以及将检查结果转换为接口响应。
"""
...
def check_daily_bar_anomalies(bars: list[Any]) -> list[Anomaly]:
    """检查日线数据中的异常值。
    ...
    for b in bars:
        ...
        if change is not None and abs(change) > 15.0:
            anomalies.append(
                Anomaly(
                    date=td,
                    code=code,
...
def check_valuation_anomalies(valuations: list[Any]) -> list[Anomaly]:
    """检查估值数据中的异常值。
    ...
    Args:
        valuations: 估值数据行列表（需有 .trade_date、.index_code、.pe、.pb、
            .pe_percentile、.pb_percentile 属性）。

...
> **策略资产原则：策略可配置资产只能是用户自行添加、且市场上存在实际 ETF
> 对应物的 A 股指数（`benchmark_index`）；申万 801xxx 行业代码与个股
> 永不作为策略资产或回测标的，只作为因子输入与研究数据。**
...
**行业/个股数据的定位**：申万一级行业（801xxx）与个股不再是策略资产域，
`StrategyConfig` 不含 `asset_domain`/`rotation`。行业日线、行业成分、个股
收盘只作为因子计算输入：RRG/扩散研究页保留独立的行业面板数据研究，
正式策略通过“作用在每个指数资产上的因子”消费这些面板（如
`rrg_industry_match_score` 内部先对行业面板做选择、再按指数成分的申万
行业暴露计算匹配分；`index_diffusion_ratio` 按指数成分股计算扩散占比），
可与动量/估值等因子在通用 score/filter/rank 中自由组合。因子元数据
四轴（category/usage/value_shape/asset_domain）与研发生命周期见
[`因子研发与集成指引.md`](因子研发与集成指引.md)。
...
│   └── builtins/                    # 内置因子计算器（指数行情/估值/宏观 + 指数级面板因子）
├── infra/db/

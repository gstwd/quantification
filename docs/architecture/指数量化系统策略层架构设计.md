...
**行业轮动（rotation）域**：`StrategyConfig.rotation` 非空时策略进入行业域
专用管线（申万一级行业，`asset_domain=industry`），由 RRG/扩散 4 个已登记
行业因子（`rrg_rs_ratio`/`rrg_rs_momentum`/`rrg_quadrant`/
`diffusion_count_ratio`）驱动 quadrant/diffusion/diffusion_rrg 信号，跳过
通用 score/filter/rank/risk；回测收敛到统一 `backtest_*` 表（`index_code`
列存放行业代码，`backtest_run.asset_domain=industry` 区分域）。因子元数据
